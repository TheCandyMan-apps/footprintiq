...
-- Add domain column to scans table for domain-based scans
ALTER TABLE public.scans 
ADD COLUMN IF NOT EXISTS domain text;

-- Add index for domain lookups
CREATE INDEX IF NOT EXISTS idx_scans_domain ON public.scans(domain) WHERE domain IS NOT NULL;

-- Update the validate_scan_input trigger to handle domain properly
