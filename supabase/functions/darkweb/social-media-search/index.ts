...
serve(async (req) => {
  ...
  try {
    ...
    if (!username || !targetId || !workspaceId) {
      return bad(400, 'Missing required fields: username, targetId, workspaceId');
    }
    ...
    if (!apifyToken) {
      console.error('[social-media-search] APIFY_API_TOKEN not configured');
    ...
    const findings = results.map((result) => ({
      ...
      is_new: true,
      severity: 'low', // Social media presence is informational
    }));
    ...
    return ok({
      ...
      findings: findings.slice(0, 10), // Return top 10
    });
