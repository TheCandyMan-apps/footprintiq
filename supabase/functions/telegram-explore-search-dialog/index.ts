...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { scan_id, dialog, query, limit, offset_id } = await req.json();
    if (!scan_id || !dialog || !query) {
      return new Response(JSON.stringify({ error: 'scan_id, dialog, and query are required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    console.error('[telegram-explore-search-dialog]', err);
